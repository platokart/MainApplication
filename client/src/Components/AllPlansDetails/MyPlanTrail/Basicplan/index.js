...
const Basicplan=()=>{
    const [invoicesHistory,setInvoiceHistory] = useState([]);
    const getPlanDetails = async() =>{
        try{
            const userId = localStorage.getItem('id');
            const response = await fetch(`http://localhost:5000/customer/profile/my-plans/${userId}`);
            if(response.ok===true){
    ...
    <div className="left-side-content-container">
        <h4>Current Plan</h4>
        <h5 className='para-basic'>Basic</h5>
        <p className='para-know-more'>Know more</p>
    </div>
