...
function ConsultantResetPassword() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (validateEmail()) {
      try {
        const response = await fetch('http://localhost:5000/consultant/forgot-password', {
          method: 'POST',
          headers: {
        ...
        navigate("/consultant/check-mail");
      } catch (error) {
...
export default ConsultantResetPassword;
