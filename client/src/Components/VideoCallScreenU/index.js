...
import NoMatchedConsultantsView from "../NoMatchedConsultantsView";
...
const VideoCallScreenU = () => {
  ...
  const [idToCall, setIdToCall] = useState(consultantEmail); // Moved to useState directly
  ...
  const mainMsg = "Please wait for the consultant to join the call"

  return (
    <VideoCallBgContainer>
      {!receivingCall ? (
            <div style={{marginTop:"40px"}}>
              <NoMatchedConsultantsView mainMsg={mainMsg} />
            </div>
      ):(
<>
      ...
      <VideoCallBgCard msgbtnClicked={isClicked}>
        <VideoCallingMainContainer msgbtnClicked={isClicked}>
          ...
          {receivingCall && (
            ...
            >
              {isVideoOn ? <IoMdVideocam /> : <FaVideoSlash />}
            </VideoCallFeaturesBtn>
            {/* Calling */}
            {callAccepted && !callEnded && (
            ...
            {receivingCall && !callAccepted && (
              ...
              // </div>
              <VideoCallBtn onClick={answerCall}>
              <IoMdCall />
            </VideoCallBtn>
            )}
            ...
            <Popup
              trigger={
                <VideoCallFeaturesBtn className="video-call-features-btn">
                  <MdSettings />
                </VideoCallFeaturesBtn>
              }
              position="top right"
...
export default VideoCallScreenU;
